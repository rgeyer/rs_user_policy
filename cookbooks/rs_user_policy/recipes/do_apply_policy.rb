...
acct_id_options = ""
logfile = ::File.join(node["rs_user_policy"]["log_dir"], "rs_user_policy-#{Time.now.to_i}.log")
...
execute "Run rs_user_policy" do
  ...
  command "rs_user_policy -r #{node["rs_user_policy"]["email"]} -s #{node["rs_user_policy"]["password"]}#{acct_id_options} -p #{policy_file} -u #{user_assignment_file} > #{logfile} 2>&1"
end

bash "Clean up old user_management*.json and log files" do
  code <<-EOF
mv #{node["rs_user_policy"]["user_assignments_dir"]}/audit* #{node["rs_user_policy"]["audits_dir"]}/
